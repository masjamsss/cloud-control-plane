#!/usr/bin/env bash
# verify.sh — acceptance harness for a new Azure environment root.
#
# The Azure sibling of importer/kit/verify.sh — provider-agnostic, so it is a near-verbatim
# reuse. The bar is EXACTLY the one the AWS prod import met:
#   1. terraform fmt -check -recursive   -> clean
#   2. terraform validate                -> clean (after offline init)
#   3. the plan gate, per phase:
#        --phase import   plan must be "N to import, 0 to add, 0 to change, 0 to destroy" —
#                         imports only, zero mutations
#        --phase steady   plan -detailed-exitcode must exit 0 (a true no-op; run after the
#                         apply + imports.tf archival)
#
# Usage:
#   verify.sh --env-dir <dir> [--phase import|steady] [--no-init] [--skip-plan]
#
# Notes
#   - fmt/validate run with `init -backend=false` (offline). The PLAN step is the only one that
#     needs the real azurerm backend + READ-ONLY Azure credentials (Reader) — run `terraform
#     init` with the backend configured first (runbook); verify.sh itself never runs a backend
#     init and NEVER applies anything.
#   - TF_BIN overrides the terraform binary — tests point it at a stub, so this script's logic
#     is fixture-testable with zero Azure/terraform (tests/test_scripts.py).
...
echo "$SUMMARY" | grep -Eq 'Plan: [0-9]+ to import, 0 to add, 0 to change, 0 to destroy' \
  || fail plan-import "plan is NOT import-only — align the config to live values until add/change/destroy are all 0 (importer/kit-azure/docs/import-plan.md step 4)"
