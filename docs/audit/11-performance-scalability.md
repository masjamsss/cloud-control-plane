...
8. **The admin subtree is code-split** (`router.tsx:32-45`) — nine lazy chunks, correctly identified as "largest, least-frequently visited."
...
- **Severity: medium**
- **Location**: `ccp/api/src/routes/admin.ts:675-689`; `ccp/api/src/domain/auditQuery.ts:36-59`

`GET /admin/audit` calls `readAuditChronological` — which loads **every** chain entry (clone per item, month-partition walk) — then `slice().reverse()`, then finds the cursor by `findIndex` (O(n)), then slices the page (`admin.ts:676-687`). The `limit` parameter (max 1000, default 100) bounds only the response body, not the server work: every page of the admin Audit History screen costs O(total chain) reads + clones + a full reverse. At a 100k-entry chain each page view is ~100k structured clones (~100-300 ms plus GC pressure) — and the SPA's per-page fetch turns paging *through* the history into repeated full-chain materialization. `GET /admin/audit/export` (`admin.ts:692-696`) legitimately needs the whole chain but additionally re-verifies every hash and builds the entire document in memory in one `c.json` — a multi-hundred-MB allocation at large chain sizes.
