...
- [x] TEST-7 | medium | contracts-docs | fixed:2045770 | 12-testing-quality.md | The SPA has no DOM/interaction testing; ~25 test files pin UI by source-string inspection
...
- [x] ARCH-9 | medium | silent-failure | fixed:7e0d449 | 01-architecture.md | Single-process, single-file scaling ceiling with in-process singletons the planned DynamoDB path would silently break
...
- [x] ERR-14 | low | silent-failure | fixed:5f36235 | 09-error-handling.md | Drift-upload compensation is non-transactional best-effort
...
- [x] CTL-5 | medium | data-persistence | fixed:2678933 | 07-catalogctl.md | `drift-edit` writes are neither atomic nor transactional: a mid-batch refusal leaves earlier edits in the checkout
- [x] DATA-10 | medium | data-persistence | fixed:12028c0 | 03-data-integrity.md | Backup/restore covers only the store JSON; the on-disk project-data/drift root it references is out of scope, with no consistency check
...
- [x] ARCH-11 | low | duplication | fixed:a87c4dd | 01-architecture.md | Arming-flag sprawl with no whole-config validation
...
- [x] FE-6 | medium | duplication | fixed:a415da1 | 05-frontend-flows.md | Api-mode submit gates read the advisory localStorage settings, not the server's — the freeze preview is dead and a stale local freeze silently blocks valid submits
...
- [x] ERR-9 | medium | frontend-ux | fixed:fa0fa9a | 09-error-handling.md | GitHub App credential fetches have no timeout, and any failure terminally fails the scan job with no retry
