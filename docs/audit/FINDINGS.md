...
- [x] API-9 | medium | stuck-state | fixed:9fd2658 | 02-api-correctness.md | Project deregistration leaves orphaned satellite rows; a reused id inherits the previous tenant's state
...
- [x] API-15 | low | stuck-state | fixed:73615f2 | 02-api-correctness.md | A dangling idempotency marker makes its key permanently unusable
- [x] DATA-12 | low | stuck-state | fixed:faafe59 | 03-data-integrity.md | Crash between the version-row transact and the file write leaves an activatable orphan row in the upload lane
...
- [x] DATA-11 | medium | authz-identity | fixed:8482cb0 | 03-data-integrity.md | v1 migration writes rows that violate the store schemas, including an `id`≠`username` shape that breaks session resolution
...
- [x] FE-9 | medium | authz-identity | fixed:9fd2658 | 05-frontend-flows.md | apiSession role resolution falls back to another scope's role when the user has no binding on the active project
- [x] CTL-9 | low | authz-identity | fixed:request.Request gains ApprovedEntries()/HasExplicitRejection(); ApprovedDigest is decision-aware; pr-prepare refuses REJECTED outright on any explicit reject and UNAPPROVED means no approve decisions | 07-catalogctl.md | `pr-prepare`'s UNAPPROVED gate accepts any non-empty approvals list without checking `decision`
...
- [x] CTL-10 | low | duplication | fixed:extracted into internal/hclobj (ParseObject/BuildObject/KeyTokens/ValueToCty); both callers reconciled onto the more careful mid-value-comment behavior; hclobj_test.go | 07-catalogctl.md | Duplicated literal-object token-walkers (edit vs driftpropose) have already diverged in behavior
...
- [x] CTL-11 | medium | test-quality | fixed:catalogsmoke_test.go dry-runs 39 (codemodOp,service) pairs against the shipped catalog, asserting never-exit-1; testdata/golden/comment-fixtures/ adds 4 end-to-end comment cases | 07-catalogctl.md | Golden coverage runs against forked fixture manifests, not the shipped catalog; comment-bearing fixtures are absent
- [x] CTL-3 | medium | test-quality | fixed:flipped waf-add-ip-set-entry/waf-remove-ip-set-entry's codemodOp to append_list_entry/remove_list_entry in the shipped catalog; foreach arity check now REFUSEs (FOREACH_ARITY) instead of exiting 1 | 07-catalogctl.md | Shipped catalog op `waf-add-ip-set-entry` can never execute (exit 1 internal error); the corrected manifest exists only in test fixtures
...
- [x] CTL-4 | medium | fail-open | fixed:R1 already fails closed; refused the shape at manifest lint time instead (RuleForeachLocalTarget) rather than loosen the shared allowSet default arm; testdata/plans/r1-fail-local-target-consumer pins today's VIOLATION; incidental fix: rds-snapshot-unshare-account was dead on arrival, verb flipped to remove_list_entry | 07-catalogctl.md | plan-check R1 structurally vetoes every legitimate plan for a `local.`-targeted foreach op
...
- [x] API-18 | low | fail-open | fixed:fb27942 | 02-api-correctness.md | Legitimize endpoint mints unlimited duplicate engineer requests for the same digest
...
- [x] CTL-2 | medium | catalogctl | fixed:movedBlock now refuses MALFORMED_NEW_NAME/ALREADY_EXISTS/DANGLING_REF before any file is touched; 3 new golden refusal cases | 07-catalogctl.md | `moved_block` writes invalid or duplicate-resource HCL at exit 0: no identifier validation, no destination-collision check, no dangling-reference handling
