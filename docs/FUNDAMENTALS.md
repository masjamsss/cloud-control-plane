...
| AGENTS.md (orientation for AI coding agents: repo map, the gates that prove a change, the machine-enforced hard rules) | [AGENTS.md](../AGENTS.md) | **Exists** (root fundamental) — a pointer file by design: it links each rule to the doc or gate that owns it and restates none of them |
