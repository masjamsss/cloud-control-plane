...
## Context
The locked [PRD](../../PRD.md) roles table (originally gerbang/CONCEPT.md lines 14–18) defines exactly three roles — Requester, Approver, Lead — with Lead = "approve and see the audit dashboard", and gerbang/BUILD-PLAN.md (`ccp/docs/history/BUILD-PLAN.md`, private planning archive — not published) (line 13) types the union accordingly, planning no admin screens. The built SPA kept the 3-role type but shipped a full admin console (users, teams, risk overrides, approval policy) gated on `isLead` (`AppShell.tsx:22`; `policy.ts` line 5: "Editable by a Lead"). Every Lead thus silently became the "highest-privilege admin" that CONCEPT.md R7 asks for — including the power to edit the very approval policy they approve under. That is the confirmed governance trap ADMIN-3 (`docs/proposals/0005-appendix/admin-backend.md`, private planning archive — not published) (critical: a single Lead can unilaterally collapse the quorum), the contradiction COHER-8 (`docs/proposals/0005-appendix/coherence.md`, private planning archive — not published), and the open decision ADMIN-9 (`docs/proposals/0005-appendix/admin-backend.md`, private planning archive — not published). With `gerbang-api` now authoritative ([0005-gerbang-approval-authority.md](0005-gerbang-approval-authority.md)), whoever holds admin holds the keys to quorum itself — so governance power must be separable from approval power without breaking the 3-role lock. Full rationale: 0005 reassessment (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published), §6 recommendation 3 "Admin-as-capability".
...
## Consequences
- Easier: ADMIN-3's dual-control design (0005 reassessment (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published) §5 item 6) has a clean subject — `isAdmin` grants are just another `PendingConfigChange`; the policy editor and the approver pool can finally be different people; ADRs 0005/0006's "lead-only enrollment" language tightens to "admin-only enrollment".
