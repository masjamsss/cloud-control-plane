...
## Context
Proposals 0002–0004 designed an "auto-eligible" standard-change lane: ops that pass a deterministic gate (reversible + LOW + non-security + no-downtime + zero-destroy + tightly-bounded + cost-neutral, 0002 §7A.3 (`docs/proposals/0002-service-module-catalog.md`, private planning archive — not published)) could apply without a human approval. 0002 qualified **36 of 74** estate Change ops (49%, "Reading the snapshot", ~line 435); 0003 generalised it across MACD to **111 of 1,171** ops (lines 11, 109), carried as an `auto_eligible` field in the op schema (0003 §1 (`docs/proposals/0003-macd-capability-matrix.md`, private planning archive — not published), ~line 45); 0004 routed on the boolean with a `plan-check` backstop (0004 (`docs/proposals/0004-frontend-interpreter-to-pr.md`, private planning archive — not published), ~line 316).

The locked concept says the opposite: `ccp/CONCEPT.md` (private planning archive — not published) flow step 5 — "**Nothing auto-applies**" (line 27). And the machinery that made the auto-lane arguably safe — ≥3-reviewer prod Environment, org/SSO identity, an implemented plan-check — does not exist; per [ADR-0005](0005-gerbang-approval-authority.md)/[ADR-0006](0006-gerbang-local-identity.md) we run an authoritative local backend on a personal repo with, today, one human approver. In that world "auto" means **zero humans**. The built app already reflects this: `autoEligible` exists only as a type field (`gerbang/app/src/types/manifest.ts:61`) and inert manifest data (`src/data/manifests/*.json`) — no routing code reads it.

A related contradiction (COHER-7 (`docs/proposals/0005-appendix/coherence.md`, private planning archive — not published)): 0003 §2 (line 78) and REDESIGN-SPEC (`ccp/docs/history/REDESIGN-SPEC.md`, private planning archive — not published) C4b (line 126) say `engineer_only` ops get "never a form", while CONCEPT.md line 24 says "never blocked at the form". The app implements the correct synthesis (`RequestForm.tsx:245` banner; `api.ts:175-178` NEEDS_ENGINEER) and this ADR ratifies it.
...
**Cons:** No fast lane if request volume grows — the pressure lands on the single approver (see the interim profile under 0005 §4 Decision B (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published)); discards design work in 0002/0003, retained only as rationale.
...
- Supersedes: 0002 (`docs/proposals/0002-service-module-catalog.md`, private planning archive — not published) §7A.1 (line 120, the auto-lane boolean), §7A.3 (eligibility gate), the `†` flags and "Reading the snapshot" (36/74, ~line 435); 0003 (`docs/proposals/0003-macd-capability-matrix.md`, private planning archive — not published) auto-lane framing (lines 11, 58–70, 109; `auto_eligible` in §1) and §2's "engineer_only must not be self-service" (line 78, per COHER-7); 0004 (`docs/proposals/0004-frontend-interpreter-to-pr.md`, private planning archive — not published) lane routing (~line 316); REDESIGN-SPEC (`ccp/docs/history/REDESIGN-SPEC.md`, private planning archive — not published) `⚡ auto` chip and `autoEligible` metadata (lines 123, 174, 250).
...
2. [ ] Prepend superseded banners to 0002 §7A and 0003 naming this ADR (banner text per `docs/proposals/0005-appendix/coherence.md` (private planning archive — not published) step 2, including the COHER-7 clause).
