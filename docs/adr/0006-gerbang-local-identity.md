...
## Context
Gerbang v2 makes `gerbang-api` the authoritative backend: approvals happen in-app and the GitHub pipeline gates become mechanical backstops ([0005-gerbang-approval-authority.md](0005-gerbang-approval-authority.md), Fork A1). Identity must therefore live where enforcement lives. Proposal 0001 assumed the opposite — §8.1 "Phase -1: the org / SSO reality that must exist before anything ships" (`docs/proposals/0001-gerbang-l1-control-plane.md`, private planning archive — not published) made a GitHub org + SAML SSO + SCIM a hard precondition, and that precondition is indefinitely blocked (no org, no IdP, fewer than 3 enrollable approvers). The repo lives on a personal account today (`github.com/masjamsss/cloud-control-plane`) and may move to an org later, so the design must not hard-depend on org-only constructs now, but must not preclude that migration. The locked concept (gerbang/CONCEPT.md (`ccp/CONCEPT.md`, private planning archive — not published), line 20) already calls for one "Gerbang bot" service account doing all git work. Full evidence: proposal 0005 (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published) and its appendix (`ADMIN-8`, `ACCT-4`, `ADV-10`).
...
## Supersedes
- Proposal 0001 §8.1 "Phase -1" (`docs/proposals/0001-gerbang-l1-control-plane.md`, private planning archive — not published) (org + SAML SSO + SCIM as a hard precondition, ~line 1442) — retired as a blocker; retained as the deferred Option B.
