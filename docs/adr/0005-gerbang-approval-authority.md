...
## Context
Gerbang turns L1 form submissions into Terraform PRs; a human approves, CI applies, nothing auto-applies. Proposal 0001 §8 (`docs/proposals/0001-gerbang-l1-control-plane.md`, private planning archive — not published) (Identity, RBAC & separation of duties, lines 1438–1660) placed all approval authority in GitHub: an org with SAML SSO (§8.1, a "hard precondition"), team CODEOWNERS (§8.2), ≥3 named humans on the prod Environment (§8.3), and an apply-job SoD check comparing GitHub logins (§8.4). None of those preconditions exist — the repo is personal (github.com/masjamsss/cloud-control-plane), there is no org, and today there is one human. The locked gerbang/CONCEPT.md (`ccp/CONCEPT.md`, private planning archive — not published) reversed this silently: local Gerbang accounts, one `gerbang-bot` doing all git. That reversal was recorded nowhere (COHER-1 (`docs/proposals/0005-appendix/coherence.md`, private planning archive — not published)) and left "who approved" living only in the app while GitHub shows the bot everywhere (ACCT-13 (`docs/proposals/0005-appendix/auditability.md`, private planning archive — not published)). The 0005 reassessment §4 Decision A (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published) framed the fork and forced this decision. Constraint: we prepare on personal GitHub now and may move to an org later — the design must not hard-depend on org-only constructs, and must not preclude that migration.
...
- **`gerbang-config` becomes security-critical.** As the authoritative approval store it needs backup/DR, PITR, an RPO/RTO target, and a degraded-mode procedure — owned by 0005 Phase 8 (`docs/proposals/0005-gerbang-v2-architecture-reassessment.md`, private planning archive — not published), not treated as cleanup.
...
4. [ ] Prepend the superseded banner to 0001 §8 (exact text in `docs/proposals/0005-appendix/coherence.md` (private planning archive — not published) step 2).
