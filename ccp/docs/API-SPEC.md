...
| POST | `/requests/:id/apply` | **lead or isAdmin** (else 403 `APPLY_FORBIDDEN`) + CSRF | **ADR-0016 approval-to-apply bundle** — the most privileged verb on this surface. Runs, server-side and in order: local gate (the plan must equal the approved change and nothing else) → CAS commit to the configured branch → satisfy the gated CI apply. It never runs `terraform apply` itself. **Off by default**: 409 `BUNDLE_DISARMED` unless `CCP_BUNDLE` + git/gate/trigger config are set. Eligible from `AWAITING_CODE_REVIEW`, `AWAITING_DEPLOY_APPROVAL` (else 409 `STATE_CONFLICT`); cooling + window settled first; refused 423 `GLOBAL_FREEZE`. The run is claimed by a CAS on `eventSeq` — the attribute the claim itself advances (ERR-11/API-4) — so a lost race is reported rather than double-run; an in-flight bundle → 409 `BUNDLE_RUNNING` while its claim is live (ERR-2 leases it, so a crashed run is taken over, not wedged). Audited `request-bundle` with full per-step evidence, written even when the request row refuses the outcome → 409 `BUNDLE_OUTCOME_CONTENDED` (CONC-6) | → `BundleOutcome` `{ok, status, bundle, steps}` — **same body on 200 and 502** | yaml `/requests/{id}/apply` : requests.ts |
| POST | `/requests/:id/cancel` | requester-own OR lead/isAdmin (else 403 `CANCEL_FORBIDDEN`) + CSRF | Cancel an approved-but-unapplied change; valid from `APPROVED_COOLING`, `AWAITING_DEPLOY_APPROVAL`, `WINDOW_EXPIRED` (requests.ts:116). Refused 409 `BUNDLE_RUNNING` while the apply bundle's claim on this request is LIVE (API-5) — an expired claim does not block it | → `ChangeRequest` | yaml:230-241 : requests.ts:836-905 |
...
| POST | `/projects/:id/onboard-tokens` | role(lead) + isAdmin; project draft/pending-trust, not archived | **(2026-07-24, easy-first-import Phase 1)** Mint a PRE-TRUST onboarding token — a separate credential/key-namespace from the CI upload token below, the EXACT INVERSE status gate (refused once trusted/ready). Shown ONCE; argon2id hash at rest; default 24h, `ttlMinutes` 5..10080. Authorizes exactly one verb: the Bearer lane on `PUT /:id/trust-request`. Audit: `onboard-token-mint`, to the TARGET chain | yaml `/projects/{id}/onboard-tokens` : projects.ts:468-516 |
| DELETE | `/projects/:id/onboard-tokens/:tokenId` | role(lead) + isAdmin | Revoke — UNLIKE the upload token, a soft-revoke: `revokedAt` is stamped and the row survives (mirrors `ProjectItem.archived`), rather than a hard delete. Audit: `onboard-token-revoke` | yaml `/projects/{id}/onboard-tokens/{tokenId}` : projects.ts:522-547 |
| PUT | `/projects/:id/trust-request` | role(lead) **OR** (2026-07-24) a Bearer onboard token — no session, no CSRF, same carve-out shape as the CI upload lane | Upload the scan artifact pair; server recomputes sha256 over the raw bytes (else 422 `PRESCAN_SHA_MISMATCH`), strict-parses `PrescanReport`, requires repo match; status → `pending-trust`; refused once trusted/ready (409). The Bearer lane runs its OWN fail-closed gate first (rate limit → lookup → project binding → not-revoked → expiry → argon2id verify → status gate, `middleware/session.ts#isOnboardTokenLane`), THEN this same pipeline, unchanged; records `uploadedBy: onboard-token:<tokenId>` and audits to the TARGET chain instead of the acting scope; accepts an optional strict `ci: {host, runUrl}` provenance block | yaml:440-458+ : projects.ts:549-678 |
| POST | `/projects/:id/trust` | role(lead) + isAdmin | ALWAYS 202 dual-control (`kind: project-trust`); body must echo the stored binding; stored bytes re-hashed; verdict ≠ clean → 422 `TRUST_VERDICT_NOT_CLEAN`. Unchanged by the token lane above — same code path regardless of which lane produced the pending-trust record | yaml:459-476 : projects.ts:679-737 |
| PUT | `/projects/:id/identity` | role(lead) + isAdmin | **(2026-07-25, ADR-0033 Decision 5)** Record the admin-confirmed cloud identity — SINGLE-ADMIN immediate write (`transactWithAudit`, mirrors register's own guard+apply shape, deliberately NOT the two-admin dual-control ceremony the trust route above uses). Body is the same provider-discriminated identity shape `POST /projects` validates (`IdentityBody`, reuses `REGION_ALLOWLIST`/`AZURE_GUID`/`AZURE_LOCATION_ALLOWLIST`/the aws-account-id regex and the same `refineIdentityShape` rule — never duplicated); overwrites the stored identity fields, explicitly clearing the other cloud's fields on a provider switch, and stamps `identityConfirmed {confirmedBy, confirmedAt}`. Callable repeatedly to correct a mistake **WHILE STILL PRE-TRUST**. **GATED to draft/pending-trust (`isOnboardable`), and refused for an archived project** — 409 `STATE_CONFLICT` otherwise (DOC-6: this row previously said "Not gated on project status or archived", the opposite of current code; identity is part of the binding the two-admin trust decision vouches for, so it takes the trust-request upload's same answer — the deliberate path post-trust is deregister + a fresh onboard). A project registered the OLD way (identity typed at register time — still how `POST /projects` works this phase) already counts as confirmed WITHOUT ever calling this route — `isIdentityConfirmed`, schema.ts:901-911. Audit: `project-identity-confirm`, to the acting scope's chain (same convention as register/trust) | yaml `/projects/{id}/identity` : projects.ts:1411-1456 |
| DELETE | `/projects/:id` | role(lead) + isAdmin | ALWAYS 202 dual-control (`kind: project-deregister`); ack deletes the registry item **and (2026-07-17) its upload tokens, data-version rows, and on-disk served data — and (2026-07-24) its onboarding tokens too**; request/audit history is never erased | yaml:491-500 : projects.ts:738-758 |
...
| Method | Path | Extra auth | Purpose | Spec : code |
|---|---|---|---|---|
| POST | `/projects/:id/upload-tokens` | role(lead) + isAdmin; project trusted/ready, not archived; **(2026-07-25, ADR-0033 Decision 5)** identity confirmed | Mint a CI upload token (shown ONCE; argon2id hash at rest; default 24h, `ttlMinutes` 5..10080). Fail-closed backstop: refuses 422 `IDENTITY_UNCONFIRMED` when `isIdentityConfirmed(project)` is false — so a project can never reach the data lane on an unconfirmed, machine-proposed identity; a project registered the old way (identity typed at register time) is unaffected | yaml `/projects/{id}/upload-tokens` : projectData.ts:108-169 (identity gate at :129-136) |
...
- POST /requests/{id}/plan-summary is implemented (ccp/api/src/routes/requests.ts:997-1048, lead-only) but absent from the YAML paths — spec lags code.
  **RESOLVED 2026-07-27 (DOC-2):** declared, with a full `PlanSummary` body schema.
- GET /admin/audit/export is implemented (ccp/api/src/routes/admin.ts:1229-1236) but absent from the YAML paths.
  **RESOLVED 2026-07-27 (DOC-2):** declared, with the `AuditExport` schema.
- GET /healthz and GET /readyz are implemented (ccp/api/src/index.ts:68-76) but absent from the YAML.
