...
| CI upload token (`ProjectUploadTokenItem`, schema.ts:1099-1110) | `PROJECT#<id>` / `UPLOADTOKEN#<tokenId>` (`uploadTokenKey()`, schema.ts:1348-1350) | `tokenId`, `projectId`, `secretHash` (argon2id), `createdBy`, `createdAt`, `expiresAt` | The credential for `PUT /projects/:id/data` (and, since the drift-portal spec, `PUT /projects/:id/drift` — same token, same trust tier). Mint requires the project be `trusted`/`ready`, not archived, AND (ADR-0033 Decision 5, 2026-07-25) identity confirmed — `isIdentityConfirmed(project)` false refuses `422 IDENTITY_UNCONFIRMED`, a fail-closed backstop so a project can never reach the data lane on an unconfirmed identity. UNLIKE the onboard token above, revoke is a hard delete, not a tombstone — there is no post-hoc audit value in a spent CI credential the way there is in an onboarding attempt's history. |
| Instance identity (`InstanceItem`, schema.ts:1027-1036) | `INSTANCE` / `META` — GLOBAL, one per store (`instanceKey()`, schema.ts:1335-1337) | `name`, `tagline`, `version`, `updatedAt`, `updatedBy` | **ADR-0023.** Seeded on first boot from `CCP_INSTANCE_NAME`/`CCP_INSTANCE_TAGLINE` (never overwrites an existing item — first-boot idempotence); edited thereafter via `PUT /admin/instance` (immediate + audited, version-guarded like `SettingItem`'s CURRENT/version pattern). Absence is a valid, meaningful state: no item yet means the SPA and the TOTP issuer both fall back to their own baked-generic default — this row is never required for the product to function. |

### 2.1a Project-scoped served data and scanning (registry-adjacent, not governance)

| Entity | Store key (PK / SK) | Key fields | Invariants and guards |
|---|---|---|---|
| Data version (`ProjectDataVersionItem`, schema.ts:1061-1092) | `PROJECT#<id>` / `DATA#v<000001>` (`projectDataVersionKey()`, schema.ts:1339-1344) | `projectId`, `version`, `uploadedAt`, `uploadedVia` (`upload-token:<tokenId>`, never a user id — this lane is CI), `digests`/`uploadDigests` (`ProjectDataDigests`, schema.ts:1038-1048 — sha256 over canonical JSON of each part), `counts`, `chunks[]` (the serve-time allowlist for `GET /projects/:id/blocks/:chunk`), `warnings[]`, `sourceCommit?`, `generatedAt?`, `providerPins?` | The registry ROW only — the actual bundle content lives on disk at `<dataRoot>/<projectId>/v<version>/`, never inside the store JSON. Versions are immutable once staged; a re-upload creates the next version rather than editing one. Whether a version is ACTIVE is derived from `ProjectItem.dataActive.version` — this row deliberately carries no mutable status of its own. |
| Scan job (`ProjectScanJobItem`, schema.ts:1155-1180) | `PROJECT#<id>` / `SCANJOB#<jobId>` | `jobId`, `projectId`, `status` (`queued\|claimed\|cloning\|scanning\|uploaded\|failed`, forward-only machine in `domain/scanner.ts`), `createdBy`, `createdAt`, `startedAt?`, `finishedAt?`, `error?` (already sanitized before write — never raw worker output), `GSI1PK`/`GSI1SK` (queue index while waiting, done index once claimed) | **ADR-0033.** Records the INTENT to scan; a separate worker clones and parses and reports back. Deliberately holds NO secret and no clone URL — the URL is rebuilt at claim time from the project's validated `RepoRef` and the upload token is minted per claim with a short TTL, so a leaked job row grants nothing. See `SCANJOB#` sweep on deregister, same as `ONBOARDTOKEN#`/`UPLOADTOKEN#` (`domain/projectsLifecycle.ts`). |
| Forge credential (`ProjectForgeCredentialItem`, schema.ts:1365-1377) | `PROJECT#<id>` / `FORGECRED` | `projectId`, `sealed` (AES-256-GCM envelope over `username:token` under `CCP_FORGE_SEAL_KEY` — a key deliberately separate from `CCP_TOTP_KEY`), `username` (clear, non-secret — lets the wizard show WHICH identity is stored without revealing the token), `createdBy`, `createdAt` | **ADR-0033 Decision 1.** The operator-supplied read-only token letting the scanner clone a PRIVATE repository. A deliberate SEPARATE row from `ProjectItem`, never a field on it — the project row is serialized by `GET /projects` on every page load, and a secret living there is one careless projection away from the wire. Nothing ever serializes this row; the registry exposes only the boolean fact one exists, and `sealed` is opened exactly once per scan job, in memory, at claim time. |

### 2.1b Drift telemetry (project-scoped; report/proposal bodies live on disk)

| Entity | Store key (PK / SK) | Key fields | Invariants and guards |
|---|---|---|---|
| Drift report version (`DriftReportItem`, schema.ts:1191-1220) | `PROJECT#<id>` / `DRIFT#v<000001>` | `projectId`, `version`, `uploadedAt`, `uploadedVia`, `envelopeDigest` (sha256 of canonical JSON of the STORED post-redaction envelope — the idempotency key), `capturedAt`, `runId`, `commit`, `cadenceHours`, `planExitCode` (`0\|2`), `counts` (`byClass`, `drifted`, `security`, `unmanaged?` — additive, OOB provisioning-import spec §3.2 rule 4) | Drift-portal spec §3.1. The registry row; the envelope body lives on disk at `<dataRoot>/<projectId>/drift/v<N>.json`, same split as `ProjectDataVersionItem`. Immutable once staged; `CCP_DRIFT_KEEP` retention pruning deletes whole rows, never edits one. `counts` is recomputed server-side from stored verdicts, never trusted from the classifier's own output. |
| Drift pointer (`DriftPointerItem`, schema.ts:1230-1243) | `PROJECT#<id>` / `DRIFT#latest` | `version`, `capturedAt`, `planExitCode`, `driftedCount`, `securityCount`, `unmanagedCount?` (additive) | Advanced in the SAME audited transaction as the version stage (drift-portal spec §3.1). Deliberately its OWN row, NOT on `ProjectItem` — telemetry must never contend with the registry's dual-controlled `version` guard. Unlike project data there is no admin activation step: drift describes reality and triggers nothing by itself, so the pointer advances the instant a report stages. |
| Drift proposal (`DriftProposalItem`, schema.ts:1262-1302) | `PROJECT#<id>` / `DRIFTPROP#<proposalDigest>` | `projectId`, `digest`, `flavor` (`adopt\|revert\|import\|restore`), `status` (`open\|submitted\|superseded`), `addresses[]`, `attrCount`, `firstReportVersion`, `lastSeenReportVersion` (bumped on every regeneration reproducing the same digest; compared against the served pointer's version at submit — `409 DRIFT_PROPOSAL_STALE` on mismatch), `baseCommit`, `generatedAt`, `requestId?` (pinned once a submit creates a request from this proposal), `arn?`/`tfType?` (import-flavor only, OOB provisioning-import spec §5.4 — the finding's own identity; informational only, NEVER read to re-derive eligibility) | Keyed by DIGEST, not by address — `flavor` distinguishes what kind of fix it proposes. `status` moves `open → submitted` (a request was created from it) or `open → superseded` (a fresher regeneration invalidated it); neither reverses. The pinned proposal BODY (not this metadata row) lives on disk, same split as the report/pointer rows above. |
