...
|---|---|---|---|
| Instance identity (`InstanceItem`, schema.ts:1021-1029) | `INSTANCE` / `META` (`instanceKey()`, schema.ts:1315-1317) | `name`, `tagline`, `version`, `updatedAt`, `updatedBy` | (DOC-12) ADR-0023 hybrid baked-default + runtime-override. Seeded AT MOST ONCE, during the installer's one ephemeral first-boot pass, from `CCP_INSTANCE_NAME`/`CCP_INSTANCE_TAGLINE` (never overwrites an existing item); edited thereafter via `PUT /admin/instance` (immediate + audited, version-guarded, `isAdmin`-only — **not** member — deliberately global, PERMISSIONS.md §2). Absence is a valid, meaningful state: no item yet ⇒ the SPA and the TOTP issuer both fall back to their own baked-generic default. |
...
| Project (`ProjectItem`, schema.ts:856-941) | `PROJECT#<id>` / `META` — global, the registry DEFINES the namespace (schema.ts:833-841, `projectKey()` schema.ts:1306) | `id`, `name`, `github {owner, repo}`, `accountId` (AWS 12-digit, schema.ts:868), `region`, `status`, `version`, `trustRequest`, `trust`, `artifacts`, **`identityConfirmed`** (`ProjectIdentityConfirmation {confirmedBy, confirmedAt}`, schema.ts:826 — ADR-0033 Decision 5, 2026-07-25; note: this row predates several OTHER additive fields — `provider`/`subscriptionId`/`tenantId`/`location`/`archived`/`dataActive` — already live in code before this addition and not individually re-verified here, a pre-existing gap this edit does not attempt to backfill) | Status is a strict forward ladder: `draft → pending-trust → trusted → ready` (`ProjectStatus`, schema.ts:764); only `ready` (and unarchived) projects join `knownProjects()` — fail closed at every earlier rung (schema.ts:833-841; ccp/api/src/projects.ts). **Data-birth (2026-07-22, [ADR-0030](../../docs/adr/0030-ccp-control-scope-and-settlement-public-summary.md)):** the old hardcoded default-project id is retired; the reserved control-plane scope `CONTROL_SCOPE = '@control'` is always known but is never a `ProjectItem` row (no registry entry ever exists for it, `GET /projects` never lists it) — see the Known-projects row in §4.3 below. **ADR-0033 Decision 5 (2026-07-25):** `identityConfirmed` is written ONLY by `PUT /projects/:id/identity` (routes/projects.ts:1411; DOC-17: was :975), its ABSENCE does not mean unconfirmed — `isIdentityConfirmed` (schema.ts:957) also treats a project carrying a full register-time identity (this phase's `POST /projects` still requires one) as confirmed, so every existing/new project this phase's register flow can produce mints CI upload tokens unaffected; see the `IDENTITY_UNCONFIRMED` row in [ERROR-STATES.md](ERROR-STATES.md). |
| Settlement marker (`SettlementItem`, schema.ts:1001-1006) | `SETTLEMENT` / `META` — GLOBAL, one per store (`settlementKey()`, schema.ts:900) | `settledAt`, `settledBy` | **New in data-birth** ([ADR-0030](../../docs/adr/0030-ccp-control-scope-and-settlement-public-summary.md)): presence means the one-time legacy-store settlement has already run (`domain/settlement.ts`) — idempotency guard, not scoped to any project. A store born entirely after this lane has nothing to settle (retro-register and materialize are both no-ops on it) but still gets the marker on first boot. |
...
| Onboarding token (`ProjectOnboardTokenItem`, schema.ts:1119-1131) | `PROJECT#<id>` / `ONBOARDTOKEN#<tokenId>` (`onboardTokenKey()`, schema.ts:1062-1064) | `tokenId`, `projectId`, `secretHash` (argon2id), `createdBy`, `createdAt`, `expiresAt`, optional `revokedAt` | **New 2026-07-24 (easy-first-import spec §3 A-ii, Phase 1).** The credential for the Bearer lane on `PUT /projects/:id/trust-request`. A SEPARATE type and key namespace from the CI upload token (`ProjectUploadTokenItem`, row below) — the two are never cross-usable (I10). Mint (`POST /projects/:id/onboard-tokens`, lead+isAdmin) is legal ONLY while `status ∈ {draft, pending-trust}` — the exact inverse of the upload token's `{trusted, ready}` gate, so the two credentials' lifetimes never overlap; refused for an archived project. UNLIKE the upload token, revoke (`DELETE .../onboard-tokens/:tokenId`) does not delete the row: it stamps `revokedAt` (mirrors `ProjectItem.archived`) so the mint survives in the audit/history trail, and the Bearer lane's fail-closed gate checks `revokedAt` as its own explicit step. Deregistering a project also sweeps its `ONBOARDTOKEN#` rows (`domain/projectsLifecycle.ts`), same as `UPLOADTOKEN#`. |
| CI upload token (`ProjectUploadTokenItem`, schema.ts:1093-1103) | `PROJECT#<id>` / `UPLOADTOKEN#<tokenId>` (`uploadTokenKey()`, schema.ts:1328-1330) | `tokenId`, `projectId`, `secretHash` (argon2id), `createdBy`, `createdAt`, `expiresAt` | (DOC-12) The credential for `PUT /projects/:id/data`. Same argon2id-at-rest, shown-once-at-mint posture as the onboarding token above, but no `revokedAt` tombstone — `DELETE /projects/:id/upload-tokens/:tokenId` deletes the row outright (immediate). Mint (`POST /projects/:id/upload-tokens`) requires trusted/ready + not archived + (ADR-0033 Decision 5) identity confirmed — 422 `IDENTITY_UNCONFIRMED` otherwise. |
| Served-data version (`ProjectDataVersionItem`, schema.ts:1055-1085) | `PROJECT#<id>` / `DATA#v<000001>` (`projectDataVersionKey()`, schema.ts:1319-1322) | `projectId`, `version`, `uploadedAt`, `uploadedVia` (`upload-token:<tokenId>`, never a user id), `digests`/`uploadDigests` (`ProjectDataDigests`: inventorySha256, blocksSha256, manifestsSha256?), `counts`, `chunks[]` (the serve-time allowlist for `GET .../blocks/:chunk`), `warnings[]`, `sourceCommit?`, `generatedAt?`, `providerPins?` | (DOC-12) The registry row; the content itself lives on disk under `<dataRoot>/<projectId>/v<version>/`, NEVER inside the store JSON. Versions are immutable once staged — a re-upload creates the NEXT version. Whether a version is ACTIVE is derived from `ProjectItem.dataActive.version`; these rows carry no mutable status of their own. `digests` vs `uploadDigests` differ only when server-side redaction changed something. |
| Scan job (`ProjectScanJobItem`, schema.ts:1149-1173) | `PROJECT#<id>` / `SCANJOB#<jobId>` (`scanJobKey()`, schema.ts:1373-1376) | `jobId`, `projectId`, `status` (`queued\|claimed\|cloning\|scanning\|uploaded\|failed`, forward-only), `createdBy`, `createdAt`, `startedAt?`, `finishedAt?`, `error?` (pre-sanitized), `GSI1PK`/`GSI1SK` (queue index) | (DOC-12) ADR-0033. The control plane records the INTENT to scan; a separate worker clones and reports back. Deliberately holds NO secret and no clone URL — the URL is rebuilt at claim time from the project's validated `RepoRef`, and the upload token is minted per claim with a short TTL, so a leaked row grants and reveals nothing about how the deployment reaches the forge. `error` is ALREADY sanitized before it is written — it originates from a worker that ran against an untrusted repository, so raw worker output must never reach this row. |
| Forge credential (`ProjectForgeCredentialItem`, schema.ts:1396-1407) | `PROJECT#<id>` / `FORGECRED` (`forgeCredentialKey()`, schema.ts:1420-1422) — at most one per project | `projectId`, `sealed` (AES-256-GCM envelope over `username:token` under `CCP_FORGE_SEAL_KEY`), `username` (clear — non-secret by construction, so the wizard can show WHICH identity is stored), `createdBy`, `createdAt` | (DOC-12) ADR-0033 Decision 1. Its own row, never a field on `ProjectItem`, for one reason: the project row is serialized by `GET /projects` on every page load, and a secret living on it is one careless projection away from the wire. Nothing serializes this row — the registry exposes only the boolean fact that one exists; the sealed blob is opened exactly once per scan job, in memory, at claim time. |
| Project retirement tombstone (`ProjectRetirementItem`, schema.ts:1348-1358) | `PROJECT#<id>` / `RETIRED` (`projectRetirementKey()`, schema.ts:1345-1347) — at most one per id, written LAST by the deregister sweep | `projectId`, `retiredAt`, `retiredBy`, `sweptRows` | (API-9) Deregistration sweeps the whole `PROJECT#<id>` partition (every satellite row it FINDS, not an enumerated prefix list — a satellite kind added tomorrow is swept for free) and writes this row last, so the partition is never empty again. `POST /projects` refuses any id whose partition still holds a row (`DUPLICATE_PROJECT`/`PROJECT_ID_RETIRED`, see [ERROR-STATES.md](ERROR-STATES.md)) — the id is retired, never recycled, which is what makes "a new tenant inherits the previous tenant's state" (e.g. a stale `ProjectForgeCredentialItem` row above) unrepresentable rather than merely unlikely. Carries no secret and no tenant data, only when and by whom, so the refusal can say something true to the admin who hits it. |
| Drift report version (`DriftReportItem`, schema.ts:1185-1213) | `PROJECT#<id>` / `DRIFT#v<000001>` (`driftVersionKey()`, schema.ts:1392-1398) | `projectId`, `version`, `uploadedAt`, `uploadedVia`, `envelopeDigest` (sha256 of the canonical stored envelope — the idempotency key), `capturedAt`, `runId`, `commit`, `cadenceHours`, `planExitCode` (`0\|2`), `counts` (`byClass`, `drifted`, `security`, `unmanaged?`) | (DOC-12) drift-portal spec §3.1. Same disk/store split as `ProjectDataVersionItem` — the envelope body lives on disk at `<dataRoot>/<projectId>/drift/v<N>.json`, never in the store JSON. Immutable once staged; `CCP_DRIFT_KEEP` retention pruning deletes whole rows, never edits one. `counts` is recomputed server-side from the stored verdicts, never trusted from the classifier's own claimed counts. |
| Drift pointer (`DriftPointerItem`, schema.ts:1224-1236) | `PROJECT#<id>` / `DRIFT#latest` (`driftPointerKey()`, schema.ts:1401-1403) | `version`, `capturedAt`, `planExitCode`, `driftedCount`, `securityCount`, `unmanagedCount?` | (DOC-12) The served-drift pointer (spec §3.1) — advanced in the SAME audited transaction as the version stage. Its OWN row, deliberately NOT on `ProjectItem`: telemetry must never contend with the registry's dual-controlled `version` guard. Unlike project data there is no admin activation step — drift describes reality and can trigger nothing by itself, so the pointer advances the instant a report stages. |
| Drift proposal (`DriftProposalItem`, schema.ts:1256-1295) | `PROJECT#<id>` / `DRIFTPROP#<digest>` (`driftProposalKey()`, schema.ts:1405-1407) | `projectId`, `digest`, `flavor` (`adopt\|revert\|import\|restore`), `status` (`open\|submitted\|superseded`), `addresses[]`, `attrCount`, `firstReportVersion`, `lastSeenReportVersion`, `baseCommit`, `generatedAt`, `requestId?`, `arn?`/`tfType?` (import-only) | (DOC-12) drift-portal spec §3.2/WI-6. Body (pinned verdict subset, diff/revert table, request skeleton) lives on disk at `<dataRoot>/<projectId>/drift/proposals/<digest>.json`. The digest IS the storage key — regenerating identical drift is an `ifNotExists` no-op; changed drift is a new row. `submitted` NEVER reverts to `open`/`superseded` — a superseded-while-submitted proposal keeps its request; the request's own gate catches staleness (§7). |

### 2.2 Project-scoped governance (every PK carries the `P#<projectId>#` prefix)

| Entity | Store key (PK / SK) | Key fields | Invariants and guards |
|---|---|---|---|
| Team (`TeamItem`, schema.ts:282-292) | `P#<pid>#TEAM#<id>` / `META` (schema.ts:584-586) | `id`, `name`, `serviceSlugs`, `version` | A service belongs to exactly one team (single ownership); server-side `stripFromOthers` removes stolen slugs in the same transaction (ccp/api/src/routes/admin.ts:717-721, :850); SPA mock enforces the same (ccp/app/src/lib/teams.ts:171-181). SPA delete guards: no members, no services (teams.ts:194-204). |
| Policy (`PolicyItem`, schema.ts:294-306) | `P#<pid>#POLICY` / `CURRENT` or `VERSION#<n>` (schema.ts:587-592) | `low`, `medium`, `high`, `deleteMin` — each int 1..5 — plus `version`, `changedBy`, `configChangeId` | Missing policy row reads as `DEFAULT_POLICY` at version 0 (ccp/api/src/domain/config.ts:14-22). Any tier decrease classifies as loosening → dual control (ccp/api/src/domain/dualControl.ts:43-48). |
| Risk override (`RiskOverrideItem`, schema.ts:308-316) | `P#<pid>#RISKOVR#<opId>` / `CURRENT` (schema.ts:593-595) | `risk: LOW\|MEDIUM\|HIGH`, `version`, `setBy`, `setAt` | Effective risk = override if set, else manifest `riskFloor` (config.ts:29-38). Lowering risk is loosening (dualControl.ts:49-50). |
| Setting (`SettingItem`, schema.ts:318-327) | `P#<pid>#SETTING#<key>` / `CURRENT` (schema.ts:596-598) | `key`, `value` (unknown), `version`, `updatedBy`, `updatedAt` | Known keys in code: `freeze.global` (config.ts:46-48), `catalog.disabled-ops` (config.ts:50-52), `rate.limits` with defaults `{submissionsPerHour: 50, maxOpen: 20}` (config.ts:54-60). |
| ChangeRequest (`RequestItem`, schema.ts:329-496) | `P#<pid>#REQ#<ulid>` / `META` (schema.ts:599-601) | `id`/`requestUlid`, `requester`, `projectId` (denormalized; the key is truth, schema.ts:244-251), `teamId`, `service`, `operationId`, `macd`, `targetAddress`, `params`, `justification`, `exposure`, `reviewTier` (pinned at submit, :260-265), `risk`, `status` (free-text machine, :267-294), `approvalsRequired`, `approvals[]`, `schedule` (`now` \| `window {at, endAt?}`, :52-60), `items[]` (change set, :320-329), `replaceConfirmation` (:302-318), `pinnedDiff`/`planDigest`/`appliedSha`/`evidenceUrl` (auto-apply pin, :330-346), `events[]`, `policyVersion`, feasibility snapshot (:352-361) | `items` present ONLY for a true set (length ≥ 2); a single-op request's top-level fields ARE the one item (schema.ts:320-327). `replaceConfirmation` must equal `targetAddress` at submit for a forces-replace op (schema.ts:310-315). A request with no intact pin can never be auto-applied — `isPinIntact` requires `sha256(pinnedDiff) === planDigest`, fail closed (ccp/api/src/domain/apply/scheduler.ts:76-90). Submit idempotency: marker at `P#<pid>#IDEMPOTENCY#<actor>#<key>` resolves a resubmit to the FIRST request (schema.ts:602-607; written ifNotExists, ccp/api/src/routes/requests.ts:434). |
| Change-set item (`RequestSetItem`, schema.ts:115-124) | embedded on `RequestItem.items[]` | `operationId`, `service`, `macd`, `targetAddress`, `params`, `exposure`, `reviewTier?`, `replaceConfirmation?` | (DOC-12) One per op in a multi-op request (Phase B); a single-op request carries NO `items` array — the request's own top-level fields (same field names) ARE the one item (`domain/changeset.ts#itemsOf` derives it either way), so every legacy row and every single-op submit stays byte-identical to before this shape existed. |
| Approval (`ApprovalItem`, schema.ts:501-506) | `P#<pid>#REQ#<ulid>` / `APPROVAL#<actorId>` (schema.ts:608-610) | `user`, `at` | One approval per actor per request, enforced by `ifNotExists` put in the approve transaction (routes/requests.ts:644). Also mirrored on `RequestItem.approvals`. |
| Request event (`RequestEventItem`, schema.ts:509-516) | `P#<pid>#REQ#<ulid>` / `EVT#<seq, zero-padded 6>` (schema.ts:611-613) | `at`, `type`, `label`, `actor?` | Timeline events; also mirrored on `RequestItem.events`. Catalog in section 5.2. |
| Pending config change (`PendingConfigChangeItem`, schema.ts:543-565) | `P#<pid>#CONFIGCHANGE#<ulid>` / `META` (schema.ts:614-616) | `kind`, `before`, `after`, `targetKey`, `apply` (`ApplySpec`, schema.ts:387-397), `proposedBy`, `status: PENDING\|APPLIED\|REJECTED\|EXPIRED\|SUPERSEDED`, `expiresAt`, `ackBy`, `ackAt` | Dual control: loosening changes need a SECOND distinct active admin (`SELF_ACK` refused, ccp/api/src/domain/dualControl.ts:233; errors.ts:22). 72h expiry (dualControl.ts:203). Drift guard: guarded attribute must still equal the propose-time value, else 409 `STALE_PROPOSAL` (dualControl.ts:239-253). `apply` never serializes to clients (`publicPendingChange`, dualControl.ts:159-178). |
| Audit entry (`AuditItem`, schema.ts:583-599) | `P#<pid>#AUDIT#<yyyymm>` / `<ulid>` (schema.ts:617-619) | `id`, `projectId`, `at`, `actor`, `action`, `targetType`, `targetId`, `before?`, `after?`, `requestId?`, `interimProfile?`, `prevHash`, `hash` | Hash-chained, tamper-evident, per-project (section 5.3). Month-partitioned; read back by walking month partitions backward until CHAINHEAD `count` entries are collected, bounded at 120 months (ccp/api/src/domain/auditQuery.ts:6-12, :48-56). |
| Chain head (`ChainHeadItem`, schema.ts:602-608) | `P#<pid>#AUDIT` / `CHAINHEAD` (schema.ts:620-622) | `hash`, `lastUlid`, `count` | Updated conditionally (`ifEquals hash`) in the SAME transaction as every audit put, so the chain cannot fork (ccp/api/src/domain/audit.ts:165-169). |
...
| Known-projects routing set | In-process CACHE of `{CONTROL_SCOPE} ∪ {store projects with status 'ready', unarchived}` — **store-only, data-birth ([ADR-0030](../../docs/adr/0030-ccp-control-scope-and-settlement-public-summary.md))**: there is no baked estate id; a blank/freshly-founded store's cache is exactly `{'@control'}` until an account clears the real onboarding ladder (or a legacy store is settled — the `SettlementItem` row above). Resynced on complete/deregister/first-activation/unarchive (ccp/api/src/projects.ts; ccp/api/src/domain/projectsLifecycle.ts) |
...
| `project-identity-confirm` | routes/projects.ts:975-1010 (2026-07-25, ADR-0033 Decision 5; `PUT /:id/identity`, lead+isAdmin) | lead | SINGLE-ADMIN immediate write (`transactWithAudit`, NOT dual control), unlike the trust row above. `before`/`after`: the provider-discriminated identity fields (`identityProjection`) plus `after` also carries `confirmedBy`/`confirmedAt`. Lands on the ACTING scope's chain — same convention as `project-register`/`project-trust-request` (session lane), not the target project's |
