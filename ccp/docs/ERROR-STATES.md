...
| `CATALOG_SKEW` | 422 | This estate's uploaded catalog and the catalog this control plane enforces describe that operation differently. Ask an admin to re-upload the estate's catalog from the version this deployment ships. | (ARCH-5) errors.ts:199-202; ccp/api/src/routes/requests.ts:389 — the project's ACTIVE served manifest (what the SPA built the form from) disagrees with the image-bundled catalog (what submit enforces, always authoritative); `details:{operationId,fields}` names the divergence. Checked before `PARAM_OUT_OF_BOUNDS`/`REPLACE_CONFIRMATION_REQUIRED` so the refusal names the real cause rather than blaming the requester for a value their own form offered |
...
| `BUNDLE_RUNNING` | 409 | A bundle for this request is already in flight. | (DOC-10) ccp/api/src/routes/requests.ts:1128 — the non-reentrancy guard on `/apply`. Also (API-5) requests.ts:1433 — `/cancel` refuses while the SAME claim is live, so the durable record cannot say CANCELLED while the bundle is irreversibly landing a commit; an EXPIRED claim (a crashed run) does not block it |
...
| `BUNDLE_OUTCOME_CONTENDED` | 409 | The bundle ran, but this request moved on while it was running … The full outcome is recorded in the audit chain — read it before re-running anything. | (CONC-6) ccp/api/src/routes/requests.ts — the bundle RAN and its outcome is in the audit chain, but the request row moved (its claim was taken over, or another verb won the row) so the row's bundle state was not updated. Carries `details:{bundle,steps}`. **This is the one 409 on this surface that must never be retried blindly** — the deploy already fired, and a retry re-runs the whole bundle. It replaces the `CHAIN_CONTENTION` this path used to emit with nothing written at all |
