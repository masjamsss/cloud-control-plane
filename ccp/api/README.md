...
| `CCP_APPLY_FROZEN` | _(unset)_ | **(DOC-9)** The 0038 auto-apply scheduler's master freeze — the operator's emergency stop. `1` halts every scheduled apply tick regardless of individual request state; unset means the scheduler runs normally whenever `CCP_SCHEDULER` is armed. This is the single knob most likely to be needed in an incident, and was previously findable only by reading `domain/apply/loop.ts`. |
| `CCP_APPLY_AUTO_REVERT` | _(unset)_ | **(DOC-9)** Companion flag to `CCP_APPLY_FROZEN`; `1` enables the scheduler's auto-revert behavior for a halted/failed auto-apply attempt. Unset = no automatic revert action is taken. |
...
| `CCP_DRIFT_IMPORT` | _(unset — **disarmed**)_ | **(DOC-9)** `1` arms the import-flavor drift proposal SUBMIT lane specifically, in addition to `CCP_DRIFT` above — a distinct, narrower gate so an operator can serve findings (and adopt/revert) without also arming import. Unset ⇒ submitting an import proposal answers `409 DRIFT_DISARMED` naming this flag. Was previously findable only in the YAML contract, not here. |
| `CCP_DRIFT_CHECK_CMD` | _(unset)_ | **(DOC-9)** Arms the drift portal's "Start drift check" button (`POST /projects/:id/drift/check`). Runs with env `CCP_DRIFT_PROJECT=<id>`; exit 0 = dispatched (fire-and-forget — the report lands later via the normal `PUT` publish, regardless of the trigger command's own exit code). Unset ⇒ the button answers `409 DRIFT_DISARMED` naming this flag. |
