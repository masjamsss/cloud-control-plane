...
const SetRoleVerb = z
  ...
  .strict();
const SetTeamVerb = z
  .object({ projectId: z.string().min(1), teamId: z.string().min(1) })
  .strict();
...
const roleRank = (r: RoleName | undefined): number =>
  r === "lead" ? 2 : r === "approver" ? 1 : 0;
...
function settingApply(
  ...
  return {
    ...
    pk: k.PK,
    sk: k.SK,
    set: { value, version: version + 1, updatedBy: actor, updatedAt: now },
    guardAttr: "version",
    guardValue: version,
  };
}

async function settingVersion(
...
export function adminRoutes(
  ...
  /* ── deployment settings — everything this system is configured with ──────
   * The point of this pair is that an operator never has to open a config file
   * to SEE how the deployment is set up, and rarely has to open one to CHANGE
   * it. GET returns every knob the server reads, including the ones the portal
   * cannot edit, each carrying a plain sentence about why and what to do
   * instead — one screen, no gaps, no pretending.
   *
   * Deployment settings are GLOBAL, so they are stored and read on the reserved
   * control scope regardless of which project the admin happens to be looking
   * at. Without that, a knob set while viewing one estate would invisibly not
   * apply while viewing another. */
  a.get("/deployment", async (c) => {
    // Secrets never carry a value here — only whether one is configured (see
    // domain/deploymentSettings.ts#knobView), so this response is safe to put
    // in a screenshot or a bug report.
    return c.json({ settings: await deploymentView(c.get("store")) });
  });

  a.put("/deployment/:id", async (c) => {
    const store = c.get("store");
    const actor = c.get("account")!.id;
    const id = c.req.param("id");
    const knob = knobById(id);
    if (!knob)
      return c.json({ code: "NOT_FOUND", reason: "No such setting." }, 404);
    ...
    if (!body || body.value === undefined)
      return apiError(c, "VALIDATION_FAILED");
    const value = body.value;
    // Shape-check against the knob's own kind — a toggle can only ever be a
    // boolean, so a string 'false' can never land and read as truthy later.
    const shapeOk =
      knob.kind === "toggle"
    ...
    if (!shapeOk)
      return apiError(c, "VALIDATION_FAILED", {
        field: "value",
        problem: `expected a ${knob.kind}`,
      });

    const key = knobSettingKey(id);
    const before = await loadSetting(store, DEPLOYMENT_SCOPE, key);
    // LOOSENING IS DUAL-CONTROLLED, by the standing machinery. Arming the
    // scanner, widening the host allowlist, or lifting the apply freeze all
    // widen what this deployment may do, so they are PROPOSED (202) and need a
    // second admin. Tightening applies at once — a safety brake must never wait.
    const classification: Classification = knob.loosens?.(before, value)
      ? "loosening"
      : "tightening";
    const version = await settingVersion(store, DEPLOYMENT_SCOPE, key);
    const apply = settingApply(
      DEPLOYMENT_SCOPE,
      key,
      value,
      version,
    ...
    const res = await commitOrPropose(store, DEPLOYMENT_SCOPE, actor, {
      classification,
      kind: `setting:${key}`,
      targetKey: `SETTING#${key}`,
      before,
      ...
      apply,
      audit: {
        action: "deployment-setting-change",
        actor,
        targetType: "setting",
        targetId: key,
        before,
    ...
    if (res.status === 200) return c.json({ ok: true, id, value });
    return c.json(publicPendingChange(res.pending), 202);
  });
  ...
  a.delete("/risk/:opId", async (c) => {
    ...
    if (res.status === 200)
      return c.json({ ok: true, opId, risk: op.riskFloor });
    return c.json(publicPendingChange(res.pending), 202);
  });
  ...
  a.put("/catalog/:opId", async (c) => {
    ...
    if (!parsed.success) return apiError(c, "VALIDATION_FAILED");
    if (!getOperation(opId, getManifests()))
      return apiError(c, "VALIDATION_FAILED");

    const before = await disabledOps(store, projectId);
    const wasDisabled = before.includes(opId);
    const after = parsed.data.enabled
      ? before.filter((x) => x !== opId)
      : [...new Set([...before, opId])];
    const classification = classify({
      target: "catalog",
      enabledBefore: !wasDisabled,
      enabledAfter: parsed.data.enabled,
    });
    const version = await settingVersion(
      store,
      projectId,
      "catalog.disabled-ops",
    );
    ...
    const res = await commitOrPropose(store, projectId, actor, {
      ...
      audit: {
        ...
        targetType: "catalog",
        targetId: opId,
        before: { enabled: !wasDisabled },
        after: { enabled: parsed.data.enabled },
      },
    });
    if (res.status === 200)
      return c.json({ ok: true, opId, enabled: parsed.data.enabled });
    return c.json(publicPendingChange(res.pending), 202);
  });
  ...
  a.post("/accounts", async (c) => {
    ...
    if (bindProject === ALL_PROJECTS)
      return apiError(c, "VALIDATION_FAILED", { field: "projectId" });
    if (!isValidProjectBinding(bindProject))
      return apiError(c, "VALIDATION_FAILED", { field: "projectId" });

    const roles: Record<string, RoleBinding> = {
      [bindProject]: { role: parsed.data.role, teamId: parsed.data.teamId },
    };
  ...
  a.patch("/accounts/:id", async (c) => {
    ...
    if (!acc)
      return c.json({ code: "NOT_FOUND", reason: "No such account." }, 404);
    ...
    if (body.displayName !== undefined) {
      if (Object.keys(body).length > 1)
        return apiError(c, "VALIDATION_FAILED", { field: "displayName" });
        ...
        [
          {
            ...
            set: {
              ...
              accountVersion: nextAccountVersion(acc),
            },
          },
        ],
        {
    ...
    const verbCount = [body.setRole, body.setTeam, body.revoke].filter(
      (v) => v !== undefined,
    ).length;
    if (verbCount > 1)
      return apiError(c, "VALIDATION_FAILED", {
        field: "setRole/setTeam/revoke",
      });

    // Every verb targets ONE registered project id; `'*'` is refused (bootstrap/migration-only).
    const verbProject =
      body.setRole?.projectId ??
      body.setTeam?.projectId ??
      body.revoke?.projectId;
    if (verbProject !== undefined) {
      if (verbProject === ALL_PROJECTS)
        return apiError(c, "VALIDATION_FAILED", { field: "projectId" });
      if (!isValidProjectBinding(verbProject))
        return apiError(c, "VALIDATION_FAILED", { field: "projectId" });
    }
    ...
    if (body.setRole) {
      ...
      verbClassification =
        roleRank(roleAfter) > roleRank(roleBefore) ? "loosening" : "tightening";
    } else if (body.setTeam) {
      const p = body.setTeam.projectId;
      if (!current[p])
        return apiError(c, "VALIDATION_FAILED", { field: "projectId" }); // no role to re-team
      ...
      if (!current[p])
        return apiError(c, "VALIDATION_FAILED", { field: "projectId" }); // not a member
      delete nextRoles[p];
    }
    ...
    const activeAdmins = accounts.filter(
      (x) => x.isAdmin && x.status === "active" && x.id !== id,
    ).length;
    const losesAdmin =
      acc.isAdmin && (body.isAdmin === false || body.status === "disabled");
    if (losesAdmin && acc.status === "active" && activeAdmins === 0)
      return apiError(c, "LAST_LEAD_GUARD");
    ...
    const otherActiveLeadsOn = (p: string): number =>
      accounts.filter(
        (x) => x.id !== id && x.status === "active" && roleFor(x, p) === "lead",
      ).length;
    const projectsLosingLead = new Set<string>();
    if (acc.status === "active" && body.status === "disabled") {
      for (const [p, b] of Object.entries(current))
        if (b.role === "lead") projectsLosingLead.add(p);
    }
    if (
      body.setRole &&
      current[body.setRole.projectId]?.role === "lead" &&
      body.setRole.role !== "lead"
    ) {
      projectsLosingLead.add(body.setRole.projectId);
    }
    if (body.revoke && current[body.revoke.projectId]?.role === "lead")
      projectsLosingLead.add(body.revoke.projectId);
    ...
    if (body.isAdmin !== undefined && body.isAdmin !== acc.isAdmin)
      classifications.push(
        classify({ target: "admin", before: acc.isAdmin, after: body.isAdmin }),
      );
    if (body.status && body.status !== acc.status)
      classifications.push(
        classify({
    ...
    const classification: Classification = classifications.some(
      (x) => x === "loosening",
    )
      ? "loosening"
      : "tightening";
    ...
    const verbPresent =
      body.setRole !== undefined ||
      body.setTeam !== undefined ||
      body.revoke !== undefined;
    const isAdminChanged =
      body.isAdmin !== undefined && body.isAdmin !== acc.isAdmin;
    const gainsSeniorCapacity =
      !isSeniorAnywhere(acc) && isSeniorAnywhere({ roles: nextRoles });
    ...
    if (body.totpRequired !== undefined)
      afterAudit.totpRequired = body.totpRequired;
  ...
  a.delete("/accounts/:id", async (c) => {
    ...
    if (acc.isAdmin && otherActiveAdmins === 0)
      return apiError(c, "LAST_LEAD_GUARD");
    ...
    for (const [p, b] of Object.entries(rolesOf(acc))) {
      if (b.role === "lead" && otherActiveLeadsOn(p) === 0)
        return apiError(c, "LAST_LEAD_GUARD");
    }
  ...
  a.post("/accounts/:id/reset-password", async (c) => {
    ...
    const classification: Classification = isSeniorAnywhere(acc)
      ? "loosening"
      : "tightening";
    const set = {
      credential: {
        algo: "argon2id",
        hash: await hashPassword(parsed.data.newPassword),
      },
      mustChangePassword: true,
  ...
  a.post("/config-changes/:id/ack", async (c) => {
    try {
      const applied = await ackPending(
        c.get("store"),
        c.get("projectId"),
        c.get("account")!.id,
        c.req.param("id"),
      ...
      await afterProjectConfigApply(
        c.get("store"),
        c.get("projectId"),
        applied,
        c.get("account")!.id,
        { dataRoot: opts.projectDataRoot },
      );
  ...
  a.get("/audit/export", async (c) => {
    ...
    c.header(
      "content-disposition",
      `attachment; filename="ccp-audit-${doc.projectId}.json"`,
    );
    return c.json(doc);
  });
  ...
  a.get("/teams", async (c) => {
    const teams = (await c
      .get("store")
      .queryGSI1(teamCollectionGsi(c.get("projectId")))) as TeamItem[];
    return c.json(
      teams
  ...
  a.post("/teams", async (c) => {
    ...
    const projectId = c.get("projectId");
    const actor = c.get("account")!.id;
    const parsed = TeamCreateBody.safeParse(
    ...
    const existing = (await store.queryGSI1(
      teamCollectionGsi(projectId),
    )) as TeamItem[];
    if (existing.some((t) => t.name.toLowerCase() === name.toLowerCase()))
      return apiError(c, "DUPLICATE_TEAM");
  ...
  a.patch("/teams/:id", async (c) => {
    ...
    const actor = c.get("account")!.id;
    const id = c.req.param("id");
    const parsed = TeamRenameBody.safeParse(
      await c.req.json().catch(() => null),
    );
    if (!parsed.success) return apiError(c, "VALIDATION_FAILED");
    const name = parsed.data.name.trim();
    ...
    if (!team)
      return c.json({ code: "NOT_FOUND", reason: "No such team." }, 404);
    if (
      existing.some(
  ...
  a.put("/teams/:id/services", async (c) => {
    ...
    const actor = c.get("account")!.id;
    const id = c.req.param("id");
    const parsed = TeamServicesBody.safeParse(
      await c.req.json().catch(() => null),
    );
    if (!parsed.success) return apiError(c, "VALIDATION_FAILED");
    ...
    await transactWithAudit(store, projectId, writes, {
      action: "team-set-services",
      actor,
      targetType: "team",
      targetId: id,
      before: {
        serviceSlugs: team.serviceSlugs,
        ...(steals.length ? { stolenFrom: steals } : {}),
      },
      after: { serviceSlugs: [...wanted] },
    });
  ...
  a.delete("/teams/:id", async (c) => {
    ...
    const members = (await loadAccounts(store)).filter(
      (acc) => teamFor(acc, projectId) === id,
    ).length;
    if (members > 0 || team.serviceSlugs.length > 0)
      return apiError(c, "TEAM_NOT_EMPTY");
  ...
  a.post("/accounts/:id/reset-totp", async (c) => {
    ...
    await transactWithAudit(
      ...
      [{ kind: "put", item: updated, ifEquals: { attr: "accountVersion", value: acc.accountVersion } }],
      {
        ...
        targetType: "account",
        targetId: id,
        before: { totpEnrolled: wasEnrolled },
        after: { totpEnrolled: false },
      },
    );
    const sessionsRevoked = await killAllSessions(store, id);
...
function stripFromOthers(
  ...
  steals: Array<{ id: string; removed: string[] }>;
} {
