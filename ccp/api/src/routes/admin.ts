...
import { exportAuditChain, readAuditPage, toAuditEntry } from '../domain/auditQuery';
...
export function adminRoutes(opts: { projectDataRoot?: string } = {}): Hono<AppEnv> {
  ...
  a.get('/audit', async (c) => {
    ...
    const { items, hasMore } = await readAuditPage(c.get('store'), c.get('projectId'), { limit, ...(cursor ? { cursor } : {}) });
