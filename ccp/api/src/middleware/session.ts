...
import { CONTROL_SCOPE, hydrateKnownProjects, isKnownProject, needsProjectHydration } from '../projects';
...
export function failCode(reason: SessionFail): 'SESSION_EXPIRED' | 'SESSION_INVALIDATED' | 'NO_SESSION' | 'TOTP_REQUIRED' {
  switch (reason) {
    case 'expired':
    case 'idle':
 ...
 */
export function isUploadTokenLane(method: string, path: string, authorization: string | undefined): boolean {
  return method === 'PUT' && /^\/projects\/[^/]+\/(data|drift)$/.test(path) && (authorization?.startsWith('Bearer ') ?? false);
}
...
export const withClientHeader: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  const exempt =
    ...
    isUploadTokenLane(method, path, c.req.header('authorization'));
...
export const withProject: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  if (!isKnownProject(projectId)) return apiError(c, 'VALIDATION_FAILED', { field: 'x-ccp-project' });
...
const PASSWORD_GATE_EXEMPT_PREFIXES = ['/auth', '/healthz', '/readyz', '/instance'];
...
export const withPasswordGate: MiddlewareHandler<AppEnv> = async (c, next) => {
  const account = c.get('account');
  const path = c.req.path;
  const exempt = PASSWORD_GATE_EXEMPT_PREFIXES.some((p) => path === p || path.startsWith(`${p}/`));
