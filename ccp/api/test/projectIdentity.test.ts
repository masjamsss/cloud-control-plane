...
import { isIdentityConfirmed, projectCollectionGsi, projectKey } from '../src/store/schema';
...
type Setup = { store: ConfigStore; app: App; putra: string; lina: string; root: string; sari: string; budi: string };
...
async function plantProjectAt(
  store: ConfigStore,
  patch: { status: ProjectItem['status']; archived?: true },
  id = 'acme',
): Promise<void> {
...
async function putIdentity(s: Setup, cookie: string, body: unknown, id = 'acme'): Promise<Response> {
  return s.app.request(`/projects/${id}/identity`, { method: 'PUT', headers: hdrs(cookie, { json: true }), body: JSON.stringify(body) });
}

async function mint(s: Setup, id = 'acme'): Promise<Response> {
  return s.app.request(`/projects/${id}/upload-tokens`, { method: 'POST', headers: hdrs(s.putra, { json: true }) });
}
...
async function plantUnconfirmedProject(store: ConfigStore, id = 'acme'): Promise<void> {
  const k = projectKey(id);
  await store.put({
    ...k,
    id,
...
describe('isIdentityConfirmed (pure predicate)', () => {
  const base = { status: 'draft', createdBy: 'x', createdAt: 'x', version: 1 } as unknown as ProjectItem;

  it('true when identityConfirmed is present, regardless of identity fields', () => {
    expect(isIdentityConfirmed({ ...base, identityConfirmed: { confirmedBy: 'a', confirmedAt: 'b' } })).toBe(true);
  });
  it('true for an aws project with accountId+region (register-time identity)', () => {
    expect(isIdentityConfirmed({ ...base, accountId: '123456789012', region: 'us-east-1' })).toBe(true);
  });
  it('false for an aws-shaped project missing region', () => {
    expect(isIdentityConfirmed({ ...base, accountId: '123456789012' })).toBe(false);
  });
  it('true for an azure project with the full subscription/tenant/location triple', () => {
    expect(
      isIdentityConfirmed({ ...base, provider: 'azure', subscriptionId: 's', tenantId: 't', location: 'eastus' }),
    ).toBe(true);
  });
  it('false for an azure project missing tenantId', () => {
    expect(isIdentityConfirmed({ ...base, provider: 'azure', subscriptionId: 's', location: 'eastus' })).toBe(false);
  });
...
describe('PUT /projects/:id/identity — validation (reuses the register validators, strictly)', () => {
  ...
  it('refuses a malformed body: bad accountId, unlisted region, mixed aws+azure fields, unknown key, missing required field', async () => {
    ...
    const bad = [
      ...
      { accountId: '123456789012', region: 'ap-southeast-1', tenantId: '11111111-2222-3333-4444-555555555555' }, // aws + azure field mixed
    ...
    for (const body of bad) {
      ...
      expect((await res.json()).code, JSON.stringify(body)).toBe('VALIDATION_FAILED');
    }
  });
  ...
  it('refuses id/name/github/repo in the body — identity-confirm can only touch identity fields', async () => {
    const s = await setup();
    await driveToTrusted(s);
    const res = await putIdentity(s, s.putra, { ...AWS_BODY, id: 'someone-else', name: 'Hijacked' });
...
describe('PUT /projects/:id/identity — happy path (single-admin, immediate, repeatable)', () => {
  ...
  it('is REPEATABLE — a second confirm overwrites confirmedBy/confirmedAt', async () => {
    ...
    const firstConfirmedAt = ((await first.json()) as { identityConfirmed: { confirmedAt: string } }).identityConfirmed.confirmedAt;

    const second = await putIdentity(s, s.root, AWS_BODY);
    expect(second.status).toBe(200);
    const body = (await second.json()) as { identityConfirmed: { confirmedBy: string; confirmedAt: string } };
  ...
  it('least disclosure: identityConfirmed is RICH-TIER only, absent from the thin ProjectSummary', async () => {
    ...
    const rich = (await (await s.app.request('/projects', { headers: hdrs(s.putra) })).json()) as Array<Record<string, unknown>>;
    expect(rich.find((p) => p.id === 'acme')?.identityConfirmed).toBeTruthy();

    const thin = (await (await s.app.request('/projects', { headers: hdrs(s.sari) })).json()) as Array<Record<string, unknown>>;
    expect(thin.find((p) => p.id === 'acme')?.identityConfirmed).toBeUndefined();
  });
});
...
describe('PUT /projects/:id/identity — lifecycle gate (settable only BEFORE the trust decision)', () => {
  ...
  it('REFUSES an archived project even at an otherwise-settable status', async () => {
    const s = await setup();
    await plantProjectAt(s.store, { status: 'draft', archived: true });
    const res = await putIdentity(s, s.putra, AWS_BODY);
    expect(res.status).toBe(409);
    expect(((await res.json()) as { code: string }).code).toBe('STATE_CONFLICT');
  });
...
describe('IDENTITY_UNCONFIRMED — the fail-closed backstop on upload-token mint', () => {
  ...
  it('a trusted project that never confirmed its identity is a DEAD END — mint refuses and the post-trust edit is refused too', async () => {
    ...
    expect(((await late.json()) as { code: string }).code).toBe('STATE_CONFLICT');
