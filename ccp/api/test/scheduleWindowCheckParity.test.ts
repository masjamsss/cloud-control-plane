 ...
 * cache, shared with createResourceParity.test.ts, instead of a fresh `go build`
 * into a throwaway temp dir on every run (see that helper's header). Invoked
 * directly — deliberately not `go run`, which prints "exit status N" to stderr and
 * always exits 1 itself on any non-zero child exit (a well-known `go run` quirk),
 * which would silently defeat an exit-code comparison.
 *
