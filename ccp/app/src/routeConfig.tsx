...
/**
 * The route tree as plain data (a `RouteObject[]`), separate from
 * `router.tsx`'s `createBrowserRouter(routeConfig)` call — UI-9's
 * regression test (`test/routeConfig.test.ts`) needs to walk this
 * structurally to confirm every entry surface sits under an
 * `errorElement`, and `createBrowserRouter` cannot be called at module
 * scope outside a browser (it reaches for `window`/`history` immediately),
 * which is exactly why this repo has no test that imports `router.tsx`
 * itself (no jsdom here — see TEST-7). Splitting the data from the
 * browser-only wiring is what makes it importable in a plain node test.
 */
 ...
 *
 * UI-6 gave every hand-rolled dialog a focus trap; UI-9 does the router
 * equivalent for entry surfaces — a pathless root layout route wrapping
 * EVERYTHING (`/login`, first-run, `/p/:projectId`, and the catch-all
 * LegacyRedirect) with one `errorElement`. Without this, only the
 * `/p/:projectId` subtree had one: a throw in `LoginPage`, `FirstRunPage`
 * (including a stale deployment's 404'd lazy-chunk load — it rejects the
 * `lazy()` promise and surfaces as a route error, not a component error),
 * or `LegacyRedirect`'s render fell through to React Router's built-in
 * "Unexpected Application Error" screen — unstyled, off-brand, and hit
 * hardest on exactly the two surfaces a user reaches *before* ever seeing
 * the app shell. The root route carries no `element`, so React Router
 * renders it as a plain `<Outlet />` (RR6's default for a childless-element
 * layout route) — this adds only the shared error boundary, no new DOM.
 */
