...
`docs/proposals/0012-review-is-the-gate-catalogctl-proposes.md` (private planning archive — not published)
...
- **`forcesReplace` and `exposure` are manifest data, not code.** Both are fields on the `Op`
  ...
  `docs/superpowers/2026-07-11-catalogctl-census.md` (private planning archive — not published)
  ...
  attributes — see
  `docs/superpowers/plans/2026-07-11-1-forcenew-verify-gate.md` (private planning archive — not published).
...
|---|---|---|
| `drift-edit` | `internal/edit` | Apply-time edit replay for a drift bundle request — the same codemod dispatch as `edit`, invoked as part of the ADR-0016 apply-to-bundle path rather than by an operator (spec). |
...
| `scan-worker` | `internal/scanworker` | The control plane's own repository scanner (ADR-0033): claims a queued scan job from the api, clones the estate repo, runs the onboard prescan+census pipeline, and uploads the result over the pre-trust onboard-token lane. The machine counterpart to a human running `onboard` locally. |
| `window-check` | `internal/windowcheck` | Maintenance-window + cooling-off gate at a supplied instant — the parity oracle the api's own `domain/schedule.ts` checks itself against, so an operator can independently verify a scheduled apply's timing without trusting the running api process. |

**Kept honest by test:** `cli_test.go`'s `TestSubcommandTableMatchesREADME` parses this table
and asserts its subcommand column is byte-set-identical to `cli.go`'s own `switch` cases — the
completeness claim above is no longer merely asserted, it fails the build if either drifts.
...
`internal/edit/edit.go` dispatches on the request's `codemodOp` through FOUR tables — this
*is* the exhaustive list (grep `dispatch`, `fileDispatch`, `instantiateHandlers`,
`createHandlers` in that file to reconfirm, rather than trust a count in prose):
...
`create_resource` (`create.go`) is the odd one out: every other verb locates an existing
block to edit; this one is the pre-locate ACCEPT branch — it has no target to find, only a
service to pick a destination file within.
...
`docs/proposals/0017-project-cleanup-plan.md` (private planning archive — not published)).
