...
// (0 ok · 2 refusal · 3 terraform/schema error · 1 internal).
func Run(opts Opts, runner Runner, stdout io.Writer) int {
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	if pathArg == "" {
		fmt.Fprintln(stderr, "usage: catalogctl onboard <path> --project-id <id> [--trusted-commit <sha>] [--out <dir>]")
		return 3
	}
	if *projectID == "" {
	...
	return Run(Opts{Root: pathArg, ProjectID: *projectID, TrustedCommit: *trusted, OutDir: *outDir}, execRunner{}, stdout)
}
