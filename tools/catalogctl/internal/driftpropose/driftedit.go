...
// pendingAdopt is one verdict that has cleared every DATA-driven gate in
// runDriftEditAdopt's phase 1 and is queued for phase 2's actual write.
type pendingAdopt struct {
	item int
	v    Verdict
}

// runDriftEditAdopt drives every item's adopt replay (spec §2-F1c), in two
// phases (CTL-5).
//
// Phase 1 gates EVERY item/verdict in the whole request and writes NOTHING:
// the digest cross-check, re-derived §6.2 eligibility, and the watchlist
// screen are all data-driven — decided entirely by the pinned request and the
// checkout's OWN watchlist FILE (loaded once, fail-closed), never by the
// target .tf files. Because none of that depends on what has or hasn't been
// written yet, it can run to completion first: a tampered digest or a
// watchlist hit on item 3 no longer leaves items 1-2's edits already applied
// — the exact "refusal-after-write" shape the finding reproduced.
//
// Phase 2 then applies every queued verdict via ApplyAdopt. One check
// CANNOT move to phase 1: adoptEdit's own checkout-dependent refinement (does
// the address still resolve, is the map missing, is a nested block
// ambiguous — "ungenerable") can only be known by actually reading the
// checkout — and ApplyAdopt deliberately re-reads disk fresh per verdict (its
// own doc comment) so that TWO verdicts touching the SAME file in one batch
// compose correctly, each seeing the prior one's write. Moving that read
// earlier would break that composition. So an ungenerable refusal in phase 2
// is the one remaining case where an earlier verdict in THIS phase already
// wrote — narrower than before (every data-driven refusal now happens before
// any write at all), not eliminated. A future caller must still not assume
// exit 2 implies an untouched checkout for that one case.
func runDriftEditAdopt(items []BundleRequestItem, repo, envDir string, stdout, stderr io.Writer) int {
	...
	// --- phase 1: gate everything, write nothing ---
	var work []pendingAdopt
	for i, item := range items {
		...
		for _, v := range p.Verdicts {
			...
			work = append(work, pendingAdopt{item: i, v: v})
		}
	}

	// --- phase 2: every gate passed; apply ---
	wrote := 0
	for _, w := range work {
		did, ungenReason, err := ApplyAdopt(w.v, envDir)
		if err != nil {
			fmt.Fprintf(stderr, "drift-edit: item %d: %s: %v\n", w.item, w.v.Address, err)
			return 1
		}
		if ungenReason != "" {
			fmt.Fprintf(stderr, "drift-edit: item %d: %s: %s\n", w.item, w.v.Address, ungenReason)
			return 2
		}
		if did {
			wrote++
...
// pendingImport is one import that has cleared every DATA-driven gate in
// runDriftEditImport's phase 1 and is queued for phase 2's actual write.
...
// runDriftEditImport drives every item's import write (spec §7.1), in the
// same two phases as runDriftEditAdopt (CTL-5) — see that function's doc
// comment for the full reasoning. Phase 1 gates every item and writes
// nothing: the digest cross-check, targetFile tamper check, re-derived §5.2
// eligibility, the watchlist's creation_security_types screen, and the
// payload prescan are all data-driven (the pinned request plus the
// checkout's own watchlist FILE, loaded once). Phase 2 applies. One check
// stays in phase 2 because it genuinely depends on the checkout's state
// AFTER earlier items in this same batch may have written: whether addr
// already resolves — the collision guard against re-importing something a
// prior item in this very request just created. An import refusal there is
// the one remaining write-after-write case; every other import refusal now
// happens before any write at all.
func runDriftEditImport(items []BundleRequestItem, repo, envDir string, stdout, stderr io.Writer) int {
	watchlist, watchlistErr := LoadWatchlist(repo)

	// --- phase 1: gate everything, write nothing ---
	var work []pendingImport
	for i, item := range items {
		var p ImportParams
		if err := json.Unmarshal(item.Params, &p); err != nil {
			fmt.Fprintf(stderr, "drift-edit: item %d: pinned params: %v\n", i, err)
		...
		work = append(work, pendingImport{item: i, addr: addr, payload: p.ImportPayload})
	}
	...
	for _, w := range work {
		...
		// re-run after a prior successful import, OR an earlier item in
		// THIS batch, lands here correctly). Genuinely checkout-dependent —
		// see this function's doc comment — so it stays here, immediately
		// before the write it gates, on fresh disk state per item.
		if _, err, _ := hclops.Locate(envDir, w.addr); err == nil {
			fmt.Fprintf(stderr, "drift-edit: item %d: %s: address already resolves in the checkout — already managed, cannot import\n", w.item, w.addr)
			return 2
		}

		// the write — fresh disk bytes per item so batched imports compose
		// (mirrors ApplyAdopt's own per-item re-read discipline).
		if err := appendImportBlock(envDir, w.payload); err != nil {
			fmt.Fprintf(stderr, "drift-edit: item %d: %s: %v\n", w.item, w.addr, err)
			return 1
		}
		wrote++
...
func appendImportBlock(envDir string, payload FindingImportPayload) error {
	...
	// CTL-5: hclops.AtomicWrite (temp + rename), not a bare os.WriteFile — see
	// ApplyAdopt's identical fix in adopt.go.
	if err := hclops.AtomicWrite(target, buf); err != nil {
